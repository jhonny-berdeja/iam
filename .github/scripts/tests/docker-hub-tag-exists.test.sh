...
if PATH="$FAKE_BIN_DIR:$PATH" DOCKERHUB_USERNAME="someuser" HUB_API_TOKEN="faketoken" "$TARGET" iam exists-tag; then
...
if PATH="$FAKE_BIN_DIR:$PATH" DOCKERHUB_USERNAME="someuser" HUB_API_TOKEN="faketoken" "$TARGET" iam missing-tag; then
...
if PATH="$FAKE_BIN_DIR:$PATH" DOCKERHUB_USERNAME="someuser" HUB_API_TOKEN="faketoken" "$TARGET" iam >/dev/null 2>&1; then
