...
if PATH="$FAKE_BIN_DIR:$PATH" DOCKERHUB_USERNAME="someuser" HUB_API_TOKEN="faketoken" "$TARGET" exists iam exists-tag; then
...
if PATH="$FAKE_BIN_DIR:$PATH" DOCKERHUB_USERNAME="someuser" HUB_API_TOKEN="faketoken" "$TARGET" exists iam missing-tag >/dev/null 2>&1; then
...
if PATH="$FAKE_BIN_DIR:$PATH" DOCKERHUB_USERNAME="someuser" HUB_API_TOKEN="faketoken" "$TARGET" not-exists iam missing-tag; then
...
if PATH="$FAKE_BIN_DIR:$PATH" DOCKERHUB_USERNAME="someuser" HUB_API_TOKEN="faketoken" "$TARGET" not-exists iam exists-tag >/dev/null 2>&1; then
...
if PATH="$FAKE_BIN_DIR:$PATH" DOCKERHUB_USERNAME="someuser" HUB_API_TOKEN="faketoken" "$TARGET" bogus iam exists-tag >/dev/null 2>&1; then
