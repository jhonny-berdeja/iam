...
const API_URL_MISSING_MESSAGE = {
  message: "El servicio de usuarios de aplicación no está disponible en este momento.",
} as const;
...
const NOT_FOUND_MESSAGE = {
  message: "El usuario de aplicación o el rol seleccionado no existen.",
} as const;
const ALREADY_ASSIGNED_MESSAGE = {
  message: "Ese rol ya está asignado a este usuario.",
} as const;
const BACKEND_ERROR_MESSAGE = {
  message:
    "El servicio de usuarios de aplicación tuvo un problema. Intentá de nuevo.",
} as const;
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const apiResponse = await assignRoleInBackend(apiUrl, token, id, body);
  if (!apiResponse) {
    return NextResponse.json(
...
async function parseRequestBody(request: NextRequest): Promise<unknown> {
  try {
    ...
    console.error(
      "Failed to parse apps-users role assignment request body",
      error,
    );
    return null;
  }
...
async function assignRoleInBackend(
  apiUrl: string,
  token: string,
  appUserId: string,
  body: unknown,
  ...
  try {
    return await fetch(`${apiUrl}/apps-users/${appUserId}/roles`, {
      method: "POST",
      headers: {
    ...
    console.error(
      "Failed to reach auth-api to assign a role to an application user",
      error,
    );
    return null;
  }
