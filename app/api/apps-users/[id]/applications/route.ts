...
const NOT_FOUND_MESSAGE = {
  message: "El usuario de aplicación o la aplicación seleccionada no existen.",
} as const;
...
async function parseRequestBody(request: NextRequest): Promise<unknown> {
  try {
    ...
    console.error(
      "Failed to parse apps-users application assignment request body",
      error,
    );
    return null;
  }
...
async function assignApplicationInBackend(
  ...
  try {
    return await fetch(`${apiUrl}/apps-users/${appUserId}/applications`, {
      method: "POST",
    ...
    console.error(
      "Failed to reach auth-api to assign an application to an application user",
      error,
    );
    return null;
  }
