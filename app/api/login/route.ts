...
export async function POST(request: NextRequest) {
  ...
  // The value auth-api checks the logging-in user's access against — same
  // header iam itself will later require on incoming requests once the
  // ADMIN guard lands there. Comes from a Secret (see infra-hub's
  // apps/iam/secret.example.yaml), never hardcoded.
  const applicationName = process.env.IAM_APPLICATION_NAME;
  if (!applicationName) {
    return NextResponse.json(
...
function buildCookieOptions() {
  return {
    httpOnly: true,
    secure: process.env.IAM_COOKIE_SECURE === "true",
    sameSite: "lax",
