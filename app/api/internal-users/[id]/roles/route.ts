...
const NOT_FOUND_MESSAGE = {
  message: "El usuario interno o el rol seleccionado no existen.",
} as const;
...
async function parseRequestBody(request: NextRequest): Promise<unknown> {
  try {
    ...
    console.error(
      "Failed to parse internal-users role assignment request body",
      error,
    );
    return null;
  }
...
async function assignRoleInBackend(
  ...
  try {
    return await fetch(`${apiUrl}/internal-users/${internalUserId}/roles`, {
      method: "POST",
      headers: {
    ...
    console.error(
      "Failed to reach auth-api to assign a role to an internal user",
      error,
    );
    return null;
  }
